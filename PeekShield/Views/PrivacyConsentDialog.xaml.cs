...
public sealed class PrivacyConsentDialog : Window
{
    ...
    public PrivacyConsentDialog(bool required)
    {
        ...
        body.Children.Add(MakeSection("二、处理方式",
            ...
            "· 如开启「刷脸解锁」，你录入的人脸特征同时可用于代替密码完成本地验证。"));
        ...
        body.Children.Add(MakeSection("六、密码保护",
            "如使用密码保护，则软件内置一个备用恢复口令机制，作为密码保护的应急通道：当您忘记自己设置的密码时，备用恢复口令可用于解锁、关闭密码保护或重置密码；其权限高于您设置的用户密码，始终可用。出于安全考虑此密码不会公开，请务必牢记您自己设置的密码，并建议设置保密问题；备用恢复口令仅作最后兜底使用。"));
        body.Children.Add(MakeSection("七、说明",
